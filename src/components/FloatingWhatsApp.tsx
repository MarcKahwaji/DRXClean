...
import socialGlobe from '@/assets/social-media-globe.png';
...
const FloatingWhatsApp = () => {
  ...
  const renderBubble = (b: typeof bubbles[0], i: number) => {
    ...
    const wrapperStyle: React.CSSProperties = {
      transform: isOpen
        ? `translate(${b.offset.x}px, ${b.offset.y}px) scale(1)`
    ...
    const inner = (
      <div
        className={`w-11 h-11 rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform ${isOpen ? 'animate-social-float' : ''}`}
        style={bubbleStyle}
      >
        {b.icon}
      </div>
